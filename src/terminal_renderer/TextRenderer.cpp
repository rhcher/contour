...
namespace
{
    ...
    StrongHash hashGlyphKeyAndPresentation(text::glyph_key const& key,
...
#else
        // TODO(pr) why's this ^^ not working as expected (hash collisions?) - but the for-loop is?
        StrongHash hash;
        hash = StrongHash(0, 0, 0, static_cast<size_t>(text.size()));
        for (auto const codepoint: text)
            hash = hash * codepoint;
        hash = hash * static_cast<uint32_t>(style);
        return hash;
#endif
    }
    ...
    StrongHash const& hash, text::glyph_key const& glyphKey, unicode::PresentationStyle presentationStyle)
{
    ...
    return textureAtlas().get_or_try_emplace(
        hash,
        [&](atlas::TileLocation tileLocation, uint32_t /*entryIndex*/)
