...
Renderer::Renderer(PageSize screenSize,
                   ...
                   uint32_t atlasTileCount,
                   Decorator hyperlinkNormal,
                   Decorator hyperlinkHover):
    _atlasDirectMapping { atlasDirectMapping },
    _atlasTileCount { std::max(atlasTileCount, static_cast<uint32_t>(screenSize.area())) },
...
{
    // clang-format off
    if (_atlasTileCount > atlasTileCount)
...
void Renderer::configureTextureAtlas()
{
    ...
    Require(atlasProperties.tileCount > 0);
