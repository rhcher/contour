...
struct StrongHash
{
    ...
    StrongHash compute(std::basic_string<T, Alloc> const& value) noexcept;
...
template <typename T>
StrongHash StrongHash::compute(std::basic_string_view<T> value) noexcept
{
    return compute(value.data(), value.size());
}

template <typename T, typename Alloc>
StrongHash StrongHash::compute(std::basic_string<T, Alloc> const& value) noexcept
{
    return compute(value.data(), value.size());
}
...
inline StrongHash operator*(StrongHash a, StrongHash b) noexcept
{
    // TODO(pr) AES-NI fallback
