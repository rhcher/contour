...
void StrongLRUHashtable<Value>::inspect(std::ostream& output) const
{
    ...
    output << fmt::format("hash table capacity : {} ({:.2}% utilization)\n",
                          _hashCount.value,
                          (double(_size) / double(_hashCount.value)) * 100.0);
    output << fmt::format("entry count         : {}\n", _size);
    output << fmt::format("entry capacity      : {} ({:.2}% utilization)\n",
                          _capacity.value,
                          (double(_size) / double(_capacity.value)) * 100.0);
    output << fmt::format("-------------------------------------------------------------\n", _name);
}
